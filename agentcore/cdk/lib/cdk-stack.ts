...
export class AgentCoreStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: AgentCoreStackProps) {
    ...
    // ---------------------------------------------------------------------------
    // SSM-backed runtime config.
    //
    // The deployed zip deliberately contains NO .env: AgentCore's CodeZip packager
    // copies codeLocation wholesale and does not read .gitignore, so credentials
    // were moved out of python-app/ (see the .gitignore comment). The runtime reads
    // them from SSM at boot instead — see python-app/runtime_secrets.py.
    //
    // ONLY THE PATH IS INJECTED HERE, NEVER THE VALUES. addEnvironmentVariable
    // writes its value verbatim into the synthesized CloudFormation template, so
    // passing secrets through it is the exact leak this design avoids. A path is
    // not a secret, so it is safe to commit and safe to sit in the template.
    //
    // No kms:Decrypt grant is needed for SecureString: the AWS-managed alias/aws/ssm
    // key's own policy already allows decrypt for this account via kms:ViaService=ssm.
    // ---------------------------------------------------------------------------
    const SECRETS_PATH = '/leadgen/';
    for (const env of this.application.environments.values()) {
      env.runtime.addEnvironmentVariable('LEADGEN_SECRETS_PATH', SECRETS_PATH);

      // Scoped to this one prefix — not ssm:* on *. The runtime has no business
      // reading any other parameter in the account.
      env.runtime.role.addToPrincipalPolicy(
        new iam.PolicyStatement({
          sid: 'ReadLeadgenSecrets',
          effect: iam.Effect.ALLOW,
          actions: ['ssm:GetParametersByPath', 'ssm:GetParameters', 'ssm:GetParameter'],
          resources: [
            `arn:aws:ssm:${this.region}:${this.account}:parameter/leadgen`,
            `arn:aws:ssm:${this.region}:${this.account}:parameter/leadgen/*`,
          ],
        }),
      );
